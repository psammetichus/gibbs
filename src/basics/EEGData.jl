...
"""
get linear combination of signals passed as a Dict of Electrode Name and weight
"""
function linearComboSignal(eeg :: EEG, weights :: Dict{String, Float64})
  signal = zeros(eeg.length)
  for s in keys(weights)
    signal += weights[s] .* getSignal(eeg, s)
  end
  return signal
end
